...
export class device_content_service {
  ...
  constructor(
    ...
    private _device_content_repository: typeof device_content,
  ) {
    this._device_content_repository = sequelize.getRepository(device_content);
  }

  private aggregate = async (device_content) => {
    ...
    return {
      name:
        product_data?.name || device_content.product.product_class.class_name,
      image:
        product_data?.image || device_content.product.product_class.class_image,
  ...
  private filter_duplicates = (device_content_dtos: device_content_dto[]) => {
  ...
  async findAll(login: string): Promise<device_content_dto[]> {
    return this._device_content_repository
      .findAll({
        raw: true,
        nest: true,
        where: {
      ...
      .then((device_contents: device_content[]) => {
        return device_contents.map(this.aggregate);
      })
  ...
  async create_device_content(
    ...
    return this._device_content_repository.create({
      device_id: device_id,
